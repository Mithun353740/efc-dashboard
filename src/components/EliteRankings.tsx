...
export default function EliteRankings() {
  ...
  return (
    <section className="py-20 px-8 max-w-7xl mx-auto">
      ...
      <div className="flex overflow-x-auto snap-x custom-scrollbar snap-mandatory sm:grid sm:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6 pb-4 sm:pb-0">
        {topPlayers.map((player, index) => (
          <Link to={`/stats?id=${player.id}`} key={player.id} className="block group w-[180px] sm:w-auto shrink-0 snap-start">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: index * 0.1 }}
              className={cn("relative rounded-2xl p-3 md:p-8 overflow-hidden transition-colors h-full", index === 0 ? "bg-brand-green/10 dark:bg-brand-green/5 border-2 border-brand-green shadow-[0_0_30px_rgba(34,197,94,0.15)] col-span-1 sm:col-span-2 lg:col-span-1" : "bg-[#f1f5f9] dark:bg-white/5 border border-transparent")}
            >
              {index === 0 && (
                <div className="absolute top-0 right-2 md:right-8 bg-brand-green text-brand-dark px-2 md:px-4 py-1 md:py-2 font-black text-[6px] md:text-[10px] tracking-widest uppercase rounded-b-xl shadow-lg z-20">
                  TOP PERFORMER
                </div>
              )}
              {/* Background Number */}
              <span className={cn("absolute -left-2 md:-left-4 -top-2 md:-top-8 text-[4rem] sm:text-[6rem] md:text-[12rem] font-black leading-none select-none", index === 0 ? "text-brand-green/20" : "text-slate-200/50 dark:text-white/5")}>
                0{index + 1}
              </span>
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center gap-1 md:gap-2 mt-1 md:mt-2 flex-wrap">
                  ...
                  <span className="bg-brand-green text-brand-dark px-1.5 md:px-2 py-0.5 md:py-1 flex items-center justify-center rounded-md text-[6px] md:text-[10px] font-black tracking-widest">
                    {player.ovr} OVR
                  </span>
                </div>
