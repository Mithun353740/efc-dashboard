import { Fixture, Team, Group } from '../types/tournament';
...
export function bergerRoundRobin(teams: Team[], legs = 1, stage = 'league', roundOffset = 0): Fixture[] {
  const list = [...teams];
  const hasBye = list.length % 2 !== 0;
  if (hasBye) list.push({ id: '__bye__', name: 'BYE' } as Team);
...
export function seededKnockout(teams: Team[], startRound = 1, legs = 1): Fixture[] {
  ...
  seeded.forEach((team, i) => { 
    if (seedOrder[i] !== undefined) slots[seedOrder[i]] = team; 
  });
...
// ─── GROUP STAGE ──────────────────────────────────────────────────────────────
export function generateGroupStage(teams: Team[], groupSize = 4): { groups: Group[], fixtures: Fixture[] } {
...
export function semiAutoFill(
  teams: Team[], 
  pinnedByRound: Record<number, Partial<Fixture>[]>, 
  totalRounds: number, 
  legs = 1, 
  stage = 'league'
): Fixture[] {
...
export function validateFixtures(fixtures: Fixture[]): string[] {
  const errors: string[] = [];
