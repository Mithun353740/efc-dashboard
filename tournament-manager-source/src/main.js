...
async function render() {
  ...
  if (state.onboarding.step > 0) {
    ...
    console.error('[KickOff] Render Error:', e.message, e.stack);
    const root = document.getElementById('root');
    if (root) root.innerHTML = `<div style="min-height:100vh;background:#050508;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:1rem;padding:2rem;text-align:center"><div style="color:#f87171;font-size:0.8rem;font-weight:900;text-transform:uppercase;letter-spacing:0.1em">Something went wrong</div><p style="color:#475569;font-size:0.7rem;max-width:400px">${e.message}</p><button onclick="location.reload()" style="margin-top:1rem;padding:0.75rem 2rem;background:linear-gradient(135deg,#3b82f6,#7c3aed);color:white;border:none;border-radius:0.75rem;font-weight:900;cursor:pointer;font-size:0.7rem;text-transform:uppercase;letter-spacing:0.1em">Reload</button></div>`;
  }
}
...
function updateScore(mId, type, val) {
  const m = state.tournament.fixtures.find(fx => fx.id === mId);
  if (m) { 
...
function toggleMatchStatus(mId) {
  const m = state.tournament.fixtures.find(fx => fx.id === mId);
